...
const submit2fa = () => {
    verifyForm.post(route('register.verify-2fa'), {
        onError: () => {
            // Keep code or reset
        }
    });
...
const resend2faCode = () => {
    ...
    resendForm.post(route('register.resend-2fa'), {
        onSuccess: () => {
            startCountdown();
        }
    });
...
<template>
    <GuestLayout :title="requires2fa ? 'Vérification de sécurité' : t.title" :subtitle="requires2fa ? 'Entrez le code de vérification pour continuer' : t.subtitle" scrollable>
