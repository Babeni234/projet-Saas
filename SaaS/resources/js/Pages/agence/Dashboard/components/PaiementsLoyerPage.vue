...
const invoices = computed(() => {
    const agencyBuildingNames = buildingsList.value;
    return rawInvoices.value.filter(i => agencyBuildingNames.includes(i.batiment));
});

const scopedPayments = computed(() => {
    const tenantNames = agencyLocataireNames.value;
    return payments.value.filter(p => tenantNames.includes(p.locataire));
});
...
// Computed KPIs
const totalCollected = computed(() => scopedPayments.value.reduce((sum, p) => sum + p.montant, 0));
const totalTransactions = computed(() => scopedPayments.value.length);
...
const topPaymentMethod = computed(() => {
    if (scopedPayments.value.length === 0) return 'Virement';
    const counts = {};
    scopedPayments.value.forEach(p => counts[p.methode] = (counts[p.methode] || 0) + 1);
    return Object.keys(counts).reduce((a, b) => counts[a] > counts[b] ? a : b);
});

const filteredPayments = computed(() => {
    let result = scopedPayments.value;
...
const savePayment = () => {
    ...
    const invoiceIndex = rawInvoices.value.findIndex(f => f.id === formData.value.factureId);
    if (invoiceIndex !== -1) {
        const inv = rawInvoices.value[invoiceIndex];
        ...
        localStorage.setItem('immobilier_factures', JSON.stringify(rawInvoices.value));
    }
...
const deletePayment = (pay) => {
    ...
    const invoiceIndex = rawInvoices.value.findIndex(f => f.id === pay.factureId);
    if (invoiceIndex !== -1) {
        const inv = rawInvoices.value[invoiceIndex];
