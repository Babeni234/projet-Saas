<template>
    <div class="flex flex-col gap-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-800">Maintenance de l'Agence</h1>
                <p class="text-sm text-slate-500 mt-1">Gérez le cycle de vie des pannes, planifiez les interventions de l'agence et suivez les budgets alloués.</p>
            </div>
            <button
        ...
        <div class="bg-white rounded-2xl p-6 shadow-lg shadow-slate-200/50 border border-slate-100">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-lg font-semibold text-slate-800">Tickets de Maintenance (Agence)</h3>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-slate-100 text-sm">
                        <tr v-for="ticket in tickets" :key="ticket.id" :class="{'bg-red-50/10': ticket.priorite === 'Critique'}">
                            ...
                            <td class="px-4 py-4">
                                <span :class="[
                                    ...
                                    ticket.priorite === 'Haute' ? 'bg-amber-50 text-amber-800 border border-amber-200' :
                        ...
                        <tr v-if="tickets.length === 0">
                            <td colspan="9" class="px-4 py-8 text-center text-slate-400 italic">Aucune maintenance enregistrée pour votre agence</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <ModalPremium
            ...
            title="Créer une tâche de maintenance (Agence)"
            subtitle="Ajoutez une nouvelle intervention sur les biens de votre agence"
            size="lg"
            type="default"
        ...
        >
            <form @submit.prevent="submitCreateForm" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Sélectionner l'Actif de l'Agence <span class="text-red-500">*</span></label>
                        <select
                ...
                <!-- Description -->
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Description de la Panne <span class="text-red-500">*</span></label>
                    <textarea
...
const priorityOptions = [
    ...
    { value: 'Critique', label: 'Critique', activeClass: 'border-red-500 bg-red-50 text-red-850' },
    { value: 'IA', label: 'Définir par l\'IA', activeClass: 'border-indigo-500 bg-indigo-50 text-indigo-850 shadow-md ring-2 ring-indigo-500/20 animate-pulse' },
];
...
const startExecution = async (ticket) => {
    try {
        ...
        if (response.ok) {
            showNotification('success', 'Intervention Débutée', 'Le statut est passé à En cours d\'exécution et le chronomètre de l\'agence a démarré.');
            await loadAllData();
        } else {
            const data = await response.json();
...
const renderCharts = () => {
    ...
    const trendValues = completedTickets.map(t => Math.round(t.duree_execution_minutes / 60 * 10) / 10);
