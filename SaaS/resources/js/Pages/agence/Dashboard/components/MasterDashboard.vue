<template>
    <div class="flex flex-col gap-8">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
                    ...
                    <div class="flex gap-4 flex-wrap lg:flex-nowrap lg:gap-6" v-if="!loading">
                        <div class="flex flex-col items-center justify-center px-4 py-3 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 min-w-[100px]">
                            ...
                            <span class="text-xs sm:text-sm text-slate-400 mt-1 font-medium">Collaborateurs</span>
                        </div>
                        <div class="flex flex-col items-center justify-center px-4 py-3 rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 min-w-[100px]">
        ...
        <div v-if="loading" class="flex flex-col items-center justify-center py-20 gap-3">
            <span class="animate-spin h-10 w-10 border-4 border-emerald-600 border-t-transparent rounded-full"></span>
            <p class="text-slate-500 text-sm font-semibold">Chargement des données de l'agence...</p>
        </div>
        ...
        <template v-else>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div class="bg-white rounded-2xl p-6 shadow-lg shadow-slate-200/50 hover:shadow-xl hover:shadow-slate-300/50 transition-all duration-300 hover:-translate-y-1 border border-slate-100">
                    ...
                    <div class="text-sm text-slate-500 mb-4">Chiffre d'Affaires de l'Agence</div>
                    <div class="pt-4 border-t border-slate-100 space-y-2">
                        <div class="flex items-center justify-between text-xs text-slate-605">
                            <span>Revenus mensuels :</span>
                            <strong class="text-slate-800 font-bold">{{ formatCurrency(kpis.revenue_actual) }}</strong>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl p-6 shadow-lg shadow-slate-200/50 hover:shadow-xl hover:shadow-slate-300/50 transition-all duration-300 hover:-translate-y-1 border border-slate-100">
                    ...
                    <div class="pt-4 border-t border-slate-100 space-y-2">
                        <div class="flex items-center justify-between text-xs text-slate-600">
                            ...
                            <strong class="text-slate-800 font-bold">{{ kpis.occupied_count }} / {{ kpis.count_logements }}</strong>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl p-6 shadow-lg shadow-slate-200/50 hover:shadow-xl hover:shadow-slate-300/50 transition-all duration-300 hover:-translate-y-1 border border-slate-100">
                    ...
                    <div class="pt-4 border-t border-slate-100 space-y-2">
                        <div class="flex items-center justify-between text-xs text-slate-600">
                            ...
                            <strong class="text-slate-800 font-bold">{{ formatCurrency(kpis.total_expenses) }}</strong>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl p-6 shadow-lg shadow-slate-200/50 hover:shadow-xl hover:shadow-slate-300/50 transition-all duration-300 hover:-translate-y-1 border border-slate-100" :class="{ 'border-l-4 border-l-amber-500': pendingExpenses.length > 0 }">
                    ...
                    <div class="text-sm text-slate-500 mb-4">Dépenses Non Validées</div>
                    <div class="pt-4 border-t border-slate-100 space-y-2">
                        <div class="flex items-center justify-between text-xs text-slate-600">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 bg-white rounded-2xl p-6 shadow-lg shadow-slate-200/50 border border-slate-100">
                    <div class="flex items-center justify-between mb-6">
                        <h3 class="text-lg font-semibold text-slate-800">Évolution Recettes vs Dépenses de l'Agence ({{ currentYear }})</h3>
                        <div class="flex items-center gap-4">
                            <div class="flex items-center gap-2">
                                <span class="w-3 h-3 rounded-full bg-emerald-500 shadow-lg shadow-emerald-500/50"></span>
                                <span class="text-xs text-slate-600 font-medium">Recettes</span>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="w-3 h-3 rounded-full bg-rose-500 shadow-lg shadow-rose-500/50"></span>
                                <span class="text-xs text-slate-600 font-medium">Dépenses</span>
                            </div>
                        </div>
                    </div>
                    <div class="h-64">
                ...
                <div class="bg-white rounded-2xl p-6 shadow-lg shadow-slate-200/50 border border-slate-100 flex flex-col justify-between">
                    <div class="mb-4">
                        <h3 class="text-lg font-semibold text-slate-800">Répartition des charges de l'Agence</h3>
                    </div>
            ...
            <!-- Unpaid Invoices and Pending Expenses sections -->
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <!-- Unpaid Invoices Section -->
                <div class="bg-white rounded-2xl p-6 shadow-lg shadow-slate-200/50 border border-slate-100 flex flex-col">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-base font-bold text-slate-800">Factures non réglées de l'Agence</h3>
                        ...
                        <table class="min-w-full divide-y divide-slate-100" v-if="unpaidInvoices.length > 0">
                            ...
                            <tbody class="divide-y divide-slate-100 bg-white">
                                <tr v-for="facture in unpaidInvoices.slice(0, 5)" :key="facture.id" class="hover:bg-slate-50/80 transition-colors">
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-xs text-slate-707">{{ facture.locataire?.nom || 'N/A' }}</td>
                ...
                <div class="bg-white rounded-2xl p-6 shadow-lg shadow-slate-200/50 border border-slate-100 flex flex-col">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-base font-bold text-slate-800">Dépenses en attente de l'Agence</h3>
                        <span class="px-2.5 py-0.5 rounded-full text-xs font-bold bg-rose-50 text-rose-700 border border-rose-100">
                            {{ pendingExpenses.length }} soumises
                        </span>
                    </div>
                    <div class="overflow-x-auto flex-1">
                        <table class="min-w-full divide-y divide-slate-100" v-if="pendingExpenses.length > 0">
                            <thead class="bg-slate-50">
                                <tr>
                                    ...
                                    <th class="px-4 py-2 text-center text-xs font-bold text-slate-500 uppercase tracking-wider">Statut</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-100 bg-white">
                        ...
                        <div class="flex flex-col items-center justify-center py-8 text-center" v-else>
                            ...
                            <p class="text-xs text-slate-400 font-medium">Aucune dépense en attente.</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-lg shadow-slate-200/50 border border-slate-100">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-base font-bold text-slate-800">Transactions Récentes de l'Agence</h3>
                    <router-link :to="{ name: 'agence.immobilier.historique' }" class="px-3 py-1.5 bg-slate-100 text-slate-707 hover:bg-slate-200 rounded-lg text-xs font-semibold transition-colors">
                        Tout Voir
                    </router-link>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-slate-100">
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            ...
                            <tr v-if="recentTransactions.length === 0">
                                <td colspan="3" class="px-6 py-12 text-center text-xs text-slate-400 italic">Aucune transaction enregistrée.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
