                            ...
                            <div class="relative group">
                                ...
                                <div class="relative">
                                    <button 
                                        ...
                                        class="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl flex justify-between items-center text-sm font-semibold text-slate-855 shadow-sm focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500"
                                    >
                                        <span>{{ selectedBuildingLabel || 'Sélectionner un bâtiment' }}</span>
                            ...
                            <div class="relative group">
                                ...
                                <div class="relative">
                                    <button 
                                        ...
                                        class="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl flex justify-between items-center text-sm font-semibold text-slate-855 shadow-sm focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 disabled:opacity-50"
                                    >
...
const typeKpis = computed(() => {
    return typeEtatDesLieux.value.map(type => {
        const count = etats.value.filter(e => e.type_etat_des_lieux_id === type.id).length;
        return {
            id: type.id,
            nom: type.nom,
            count: count
...
const filteredFormBuildings = computed(() => {
    if (!formBuildingSearchQuery.value) return batiments.value;
    const query = formBuildingSearchQuery.value.toLowerCase();
    return batiments.value.filter(b => b.nom.toLowerCase().includes(query));
});
