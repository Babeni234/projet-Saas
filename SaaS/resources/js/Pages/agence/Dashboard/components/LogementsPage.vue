    ...
    <div class="flex flex-col gap-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-slate-600 mt-1">Gérer le parc immobilier de l'agence, suivre l'occupation et les revenus de location.</p>
            </div>
            <button
                @click="openAddModal"
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr v-if="filteredLogements.length === 0">
                            <td colspan="7" class="text-center py-12 text-slate-400">
                                Aucun bien immobilier trouvé dans la base de données de l'agence.
                            </td>
                        </tr>
                    </tbody>
                </table>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option v-for="b in filteredBatiments" :key="b.id" :value="b.id">
        ...
        <div v-if="showSuccess" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 animate-scale-up">
                ...
                <p class="text-center text-slate-555 text-sm mb-6">L'unité immobilière a bien été actualisée.</p>
...
const filteredBatiments = computed(() => {
    ...
    if (agencyId) {
        return systemBatiments.value.filter(b => Number(b.agency_id) === Number(agencyId));
    }
    return systemBatiments.value;
});
...
const openAddModal = () => {
    ...
    formData.value = {
        ...
        loyer: '',
        statut: 'Libre',
        agency_id: currentAgencyId.value || null
    };
...
const openEditModal = (logement) => {
    ...
    formData.value = {
        ...
        agency_id: logement.agency_id || currentAgencyId.value || null
    };
    showModal.value = true;
};
...
const onBatimentChange = () => {
    if (formData.value.batiment_id) {
        ...
        if (selectedBat && selectedBat.agency_id) {
            ...
            formData.value.agency_id = currentAgencyId.value || null;
        }
    } else {
        formData.value.agency_id = currentAgencyId.value || null;
    }
...
const saveLogement = async () => {
    ...
    const payload = {
        ...
        agency_id: formData.value.agency_id ? Number(formData.value.agency_id) : (currentAgencyId.value || null),
    };
