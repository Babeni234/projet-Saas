...
// Dynamic buildings list fetched from system localStorage, filtered by agency
...
const filteredLogements = computed(() => {
    // Filter by agency buildings
    const agencyBuildingNames = systemBatiments.value.map(b => b.nom);
    let list = logements.value.filter(l => agencyBuildingNames.includes(l.batiment));
