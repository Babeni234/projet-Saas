...
// Dynamic list of agency tenants to choose as clients
...
const clients = computed(() => {
    return systemLocataires.value.map((l, index) => ({ 
        id: index + 1, 
        nom: l.nom,
        email: l.email || '',
...
watch(() => facture.value.clientId, (newVal) => {
    if (newVal) {
        const cl = clients.value.find(c => c.id === newVal);
        if (cl) {
            facture.value.clientEmail = cl.email;
            facture.value.clientAdresse = cl.telephone;
        }
    }
});
...
const closeSuccess = () => {
    ...
    router.push({ name: 'agence.accounting.factures' });
};
</script>
