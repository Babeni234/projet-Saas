        ...
        <div v-if="showModal || isFullScreenForm" :class="isFullScreenForm ? 'flex flex-col h-full animate-scale-up' : 'fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4'">
            <div :class="isFullScreenForm ? 'w-full h-full bg-white flex flex-col' : 'bg-white rounded-3xl shadow-2xl max-w-4xl w-full overflow-hidden flex flex-col max-h-[90vh] animate-scale-up'">
                ...
                <div class="p-6 overflow-y-auto flex-1 space-y-6">
                    <!-- Agency, Building, Tenant & Contract selection cascade -->
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                            ...
                            >
                                ...
                                <option v-for="a in dbAgencies" :key="a.id" :value="a.id">{{ a.name || a.nom }}</option>
                            </select>
                        </div>

                        <!-- Building -->
                        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm" v-if="formData.agency_id">
                            ...
                            >
                                ...
                                <option v-for="b in filteredBatiments" :key="b.id" :value="b.id">{{ b.nom }}</option>
                            </select>
                        </div>
...
const enableFullScreen = () => {
    showModal.value = false; // it's mutually exclusive layout wise in our UI
