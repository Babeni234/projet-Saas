...
const facturesPayees = computed(() => factures.value.filter(f => f.statut === 'Payée').length);
const facturesEnAttente = computed(() => factures.value.filter(f => f.statut === 'Envoyée').length);
const facturesEnRetard = computed(() => factures.value.filter(f => f.statut === 'En retard').length);

const createNewFacture = () => {
    router.push({ name: 'accounting.facture-creation' });
};
...
const editFacture = (facture) => {
    router.push({ name: 'accounting.facture-creation', params: { id: facture.id } });
};
