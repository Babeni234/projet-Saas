<template>
    <div class="flex flex-col gap-6">
        ...
        <!-- Multiple Templates Configuration Card -->
        <div class="bg-white rounded-2xl shadow-lg shadow-slate-200/50 p-6 border border-slate-100">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                <div>
                    <h3 class="text-lg font-bold text-slate-900 flex items-center gap-2">
                        ...
                        Modèles de Contrat par Type de Bail
                    </h3>
                    <p class="text-sm text-slate-500 mt-1">Configurez un modèle de contrat spécifique pour chaque type de bail existant dans votre entreprise.</p>
                    <p class="text-xs text-indigo-600 font-semibold mt-1">Bailleur configurateur : {{ companyName }}</p>
                </div>
                <div class="flex items-center gap-3">
                    <select 
                        v-model="selectedTemplateTypeContratId"
                        @change="onTemplateTypeChange"
                        class="px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent bg-white text-sm"
                    >
                    ...
                    <button 
                        @click="saveTemplate" 
                        :disabled="!selectedTemplateTypeContratId || savingTemplate"
                        class="px-4 py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-sm font-medium shadow transition disabled:opacity-50 flex items-center gap-2"
                    >
                        <svg v-if="savingTemplate" class="animate-spin h-4 w-4 text-white" fill="none" viewBox="0 0 24 24">
            ...
            <textarea 
                ...
                :disabled="!selectedTemplateTypeContratId"
                rows="8" 
                class="w-full p-4 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent font-mono text-sm text-slate-700 bg-slate-50 disabled:bg-slate-100 disabled:cursor-not-allowed"
                placeholder="Sélectionnez d'abord un type de contrat ci-dessus pour rédiger son modèle..."
            ></textarea>
        </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div v-if="!editingContrat" class="space-y-4">
                                <!-- 1. Agency Select -->
                                <div>
                                    <label class="block text-xs font-semibold text-slate-500 mb-1">Agence</label>
                                    <select 
...
const filteredBuildings = computed(() => {
    ...
    return batiments.value.filter(b => Number(b.agency_id) === Number(selectedAgencyId.value));
});
...
const fetchData = async () => {
    loading.value = true;
    try {
        const [contratsRes, agenciesRes, batimentsRes, affectationsRes, typeContratsRes, templatesRes] = await Promise.all([
            axios.get(route('contrats.json')),
            axios.get('/agencies', { headers: { Accept: 'application/json' } }).catch(() => ({ data: { agencies: { data: [] } } })),
        ...
        agencies.value = agenciesRes.data?.agencies?.data || [];
...
const selectedTemplateTypeContratId = ref('');
const exemplaireContrat = ref('');

const onTemplateTypeChange = () => {
    const matched = templates.value.find(t => Number(t.type_contrat_id) === Number(selectedTemplateTypeContratId.value));
    if (matched) {
        exemplaireContrat.value = matched.content;
    } else {
        exemplaireContrat.value = `CONTRAT DE BAIL D'HABITATION\n\nEntre les soussignés :\nLe Bailleur : ${companyName.value}\nEt\nLe Preneur : [Locataire]\n\n1. Désignation : Logement [Logement] dans le bâtiment [Bâtiment].\n2. Loyer : [Loyer] € / Caution : [Caution] €.\n3. Durée : [Durée] à partir du [Date Début].\n\nFait le [Date]`;
    }
};

const saveTemplate = async () => {
    if (!selectedTemplateTypeContratId.value) return;
    savingTemplate.value = true;
    try {
        const res = await axios.post(route('contrat-templates.storeOrUpdate'), {
            type_contrat_id: selectedTemplateTypeContratId.value,
            content: exemplaireContrat.value,
        });
        if (res.data.success) {
            // Update local templates list
            const index = templates.value.findIndex(t => Number(t.type_contrat_id) === Number(selectedTemplateTypeContratId.value));
            if (index !== -1) {
                templates.value[index] = res.data.template;
            } else {
                templates.value.push(res.data.template);
            }
...
const saveContrat = async () => {
    ...
    try {
        ...
        // Refresh local database entries to capture state changes
        fetchData();
    } catch (e) {
        console.error(e);
