<template>
    <div class="space-y-8 max-w-5xl mx-auto pb-12 text-left">
        ...
        <Transition name="fade">
            <div v-if="successToast" class="fixed bottom-6 right-6 z-50 flex items-center gap-3 rounded-2xl bg-emerald-500 px-5 py-4 text-white shadow-xl shadow-emerald-500/20 border border-emerald-400/30">
        ...
        <!-- Header Hero Card -->
        <div class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-indigo-950 via-slate-900 to-indigo-900 p-8 md:p-10 text-white shadow-2xl border border-slate-800">
            <div class="absolute -right-20 -top-20 h-72 w-72 rounded-full bg-gradient-to-br from-indigo-500/30 to-purple-600/30 blur-3xl"></div>
            <div class="absolute -left-20 -bottom-20 h-72 w-72 rounded-full bg-gradient-to-tr from-violet-500/20 to-indigo-600/20 blur-3xl"></div>
            ...
            <div class="relative z-10 space-y-6">
                <div class="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div class="bg-white/5 backdrop-blur-md rounded-2xl p-4 border border-white/10 shadow-lg shrink-0 min-w-[200px]">
                        <p class="text-xs text-indigo-250 font-bold uppercase tracking-wider mb-1">Votre forfait actuel</p>
                        <h2 class="text-xl font-black text-white mb-2">
                            {{ currentPlanName }}
                        </h2>
                        <div class="flex items-baseline gap-1 text-amber-400 font-extrabold">
                            <span>{{ formatPrice(currentPlanPrice) }}</span>
        ...
        <div class="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-slate-200/60 relative overflow-hidden">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <!-- Logements Limit -->
                <div class="bg-slate-50/60 rounded-2xl p-5 border border-slate-100 flex flex-col justify-between">
                    <div>
                    ...
                    <div class="mt-4 w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                        <div class="bg-gradient-to-r from-indigo-500 to-indigo-650 h-full rounded-full" :style="{ width: currentPlan?.max_logements === -1 ? '100%' : '50%' }"></div>
                    </div>
                </div>
                ...
                <div class="bg-slate-50/60 rounded-2xl p-5 border border-slate-100 flex flex-col justify-between">
                    ...
                    <div class="mt-4 w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                        <div class="bg-gradient-to-r from-indigo-500 to-indigo-650 h-full rounded-full" :style="{ width: currentPlan?.max_locataires === -1 ? '100%' : '50%' }"></div>
                    </div>
                </div>
                ...
                <div class="bg-slate-50/60 rounded-2xl p-5 border border-slate-100 flex flex-col justify-between">
                    ...
                    <div class="mt-4 w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                        <div class="bg-gradient-to-r from-indigo-500 to-indigo-650 h-full rounded-full" :style="{ width: currentPlan?.max_employees === -1 ? '100%' : '50%' }"></div>
                    </div>
                </div>
                ...
                <div class="bg-slate-50/60 rounded-2xl p-5 border border-slate-100 flex flex-col justify-between">
                    ...
                    <div class="mt-4 w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                        <div class="bg-gradient-to-r from-indigo-500 to-indigo-650 h-full rounded-full" :style="{ width: currentPlan?.max_agencies === -1 ? '100%' : '50%' }"></div>
                    </div>
                </div>
                ...
                <div class="bg-slate-50/60 rounded-2xl p-5 border border-slate-100 flex flex-col justify-between">
                    ...
                    <div class="mt-4 w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                        <div class="bg-gradient-to-r from-indigo-500 to-indigo-650 h-full rounded-full" :style="{ width: currentPlan?.max_buildings === -1 ? '100%' : '50%' }"></div>
                    </div>
                </div>
                ...
                <div class="bg-slate-50/60 rounded-2xl p-5 border border-slate-100 flex flex-col justify-between">
                    <div>
                        ...
                        <h3 class="text-2xl font-black flex items-center gap-2" :class="currentPlan?.has_ai ? 'text-emerald-600' : 'text-slate-550'">
        ...
        <div class="space-y-6">
            <div class="text-center md:text-left">
                ...
                <p class="text-sm text-slate-550">Les forfaits disponibles sont conçus pour évoluer parallèlement à votre activité.</p>
            </div>
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div
                    ...
                    :key="plan.slug"
                    class="relative bg-white rounded-3xl border-2 transition-all duration-300 flex flex-col justify-between overflow-hidden group shadow-lg"
                    :class="[
                        plan.slug === currentPlanSlug 
                            ? 'border-indigo-500 ring-2 ring-indigo-500/20' 
                            : (plan.popular ? 'border-amber-400 ring-2 ring-amber-400/20' : 'border-slate-200/80 hover:border-indigo-300 hover:shadow-xl')
                    ]"
                >
                    <!-- Popular / Current Tag banner -->
                    <div
                        v-if="plan.slug === currentPlanSlug"
                        class="absolute top-0 right-0 bg-indigo-500 text-white text-[10px] font-black uppercase tracking-wider px-4 py-1 rounded-bl-xl shadow-sm"
                    >
                        Actuel
                    </div>
                    <div
                        v-else-if="plan.popular"
                        class="absolute top-0 right-0 bg-gradient-to-r from-amber-500 to-amber-600 text-white text-[10px] font-black uppercase tracking-wider px-4 py-1 rounded-bl-xl shadow-sm"
                    >
                        Recommandé
                    </div>
                    ...
                    <div class="p-6 md:p-8 flex-1 flex flex-col justify-between">
                        <div>
                            ...
                            <div class="mb-6">
                                ...
                                <p class="text-xs text-slate-500 mt-1 uppercase tracking-wide font-medium">Cycle mensuel</p>
                            </div>
                            ...
                            <div class="flex items-baseline gap-1.5 mb-8">
                                ...
                                <span class="text-xs font-extrabold text-slate-500">FCFA / mois</span>
                            </div>
                            ...
                            <div class="space-y-4 mb-8">
                                ...
                                <ul class="space-y-3">
                                    <li class="flex items-center gap-2.5 text-xs text-slate-650 font-medium">
                                        <svg class="w-4 h-4 text-indigo-500 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                            ...
                            <div class="space-y-3 mb-6">
                                ...
                                <ul class="space-y-2">
                                    <li v-for="feat in parseFeatures(plan.features)" :key="feat" class="flex items-start gap-2 text-xs text-slate-600 leading-normal">
                                        <svg class="w-4.5 h-4.5 text-emerald-500 shrink-0 mt-0.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                        ...
                        <div class="mt-8">
                            <button
                                v-if="plan.slug === currentPlanSlug"
                                disabled
                                class="w-full py-3.5 px-4 rounded-2xl bg-indigo-50 text-indigo-600 font-extrabold text-xs shadow-sm border border-indigo-200/50 cursor-not-allowed text-center transition-colors"
                            >
                                Votre Plan Actuel
                            </button>
                            <button
                                ...
                                class="w-full py-3.5 px-4 rounded-2xl bg-gradient-to-r from-indigo-500 to-indigo-650 hover:from-indigo-600 hover:to-indigo-700 text-white font-extrabold text-xs shadow-lg shadow-indigo-500/10 hover:shadow-xl hover:shadow-indigo-500/20 text-center transition-all duration-200"
                            >
        ...
        <Transition name="fade">
            <div v-if="confirmModalOpen" class="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
                <div class="bg-gradient-to-br from-white via-white to-indigo-50/10 rounded-3xl shadow-2xl max-w-lg w-full p-8 border border-indigo-100/50 relative overflow-hidden animate-scale-up text-left">
                    ...
                    <div class="relative z-10 space-y-6">
                        <!-- Icon Header -->
                        <div class="flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-indigo-500 to-indigo-650 shadow-lg shadow-indigo-500/30">
                            <svg class="w-8 h-8 text-white animate-pulse" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        ...
                        <!-- Info -->
                        <div>
                            <h3 class="text-xl font-black text-slate-800">Confirmer le changement d'abonnement</h3>
                            <p class="text-slate-500 text-xs mt-1">Vous vous apprêtez à migrer vers une offre supérieure.</p>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                            ...
                            <div class="border-l border-slate-200 pl-4">
                                ...
                                <p class="text-xs font-black text-indigo-600 mt-0.5">{{ formatPrice(targetPlan?.price) }} FCFA</p>
                            </div>
                        </div>

                        <!-- Alert message -->
                        <div class="rounded-xl bg-indigo-50/50 border border-indigo-100 p-3 text-xs text-slate-600 leading-normal flex gap-2">
                            <svg class="w-5 h-5 text-indigo-500 shrink-0 mt-0.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            ...
                            <span>
                                En confirmant, votre compte sera immédiatement mis à jour avec les nouvelles capacités de logements, d'employés et d'IA. Votre nouveau cycle de facturation commence aujourd'hui.
                            </span>
                        </div>

                        <!-- Error Message inside modal -->
                        <p v-if="modalError" class="text-red-500 text-xs font-semibold bg-red-50 border border-red-200 rounded-xl p-3">
                            {{ modalError }}
                        </p>
                        ...
                        <div class="flex gap-4">
                            ...
                            <button
                                ...
                                :disabled="submittingUpgrade"
                                class="flex-1 px-5 py-3.5 bg-gradient-to-r from-indigo-500 to-indigo-650 text-white rounded-xl font-bold shadow-lg shadow-indigo-500/30 hover:shadow-xl hover:shadow-indigo-500/40 transition-all text-xs disabled:opacity-50 flex items-center justify-center gap-2"
                            >
                                <svg v-if="submittingUpgrade" class="animate-spin h-4 w-4 text-white" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                </svg>
                                <span>{{ submittingUpgrade ? 'Mise à niveau...' : 'Confirmer & Payer' }}</span>
                            </button>
                        </div>
                    </div>
                </div>
...
const executeUpgrade = async () => {
    ...
    try {
        ...
        if (response.data.success) {
            ...
            // Reload page to refresh Inertia shared data
            setTimeout(() => {
                successToast.value = false;
                // Force Inertia reload to sync the page auth properties reactively
                router.reload({
                    only: ['auth'],
                    onSuccess: () => {
                        // Redirect to master dashboard or just keep on this page showing current active plan updated
                        router.visit('/dashboard/company/profile');
                    }
                });
            }, 2500);
        } else {
            modalError.value = response.data.error || 'Une erreur est survenue.';
        }
    } catch (err) {
        console.error(err);
        modalError.value = err.response?.data?.error || 'Une erreur est survenue lors de la mise à niveau.';
    } finally {
