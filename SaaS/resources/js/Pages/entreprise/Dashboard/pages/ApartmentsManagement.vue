    ...
    <div class="space-y-8">
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-4">
            <div class="enterprise-card group p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-slate-500 text-sm font-semibold uppercase tracking-wider">Total Logements</p>
                    ...
                    <div class="w-16 h-16 bg-gradient-to-br from-blue-500 via-blue-600 to-indigo-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/30 group-hover:scale-110 transition-transform duration-300">
                        <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2.422-7.267A2 2 0 017.35 3h9.3a2 2 0 011.928 1.733L21 12M3 12a9 9 0 0118 0m-9 9v-6m4 6v-6m-4-6h.01M12 3h.01" />
                        </svg>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-white to-emerald-50/30 rounded-3xl shadow-xl shadow-emerald-200/50 p-8 border border-emerald-100/50 hover:shadow-2xl hover:shadow-emerald-200/60 transition-all duration-500 group">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-slate-500 text-sm font-semibold uppercase tracking-wider">Occupés</p>
                        <p class="text-4xl font-bold bg-gradient-to-r from-emerald-600 to-emerald-500 bg-clip-text text-transparent mt-3">{{ stats.occupied }}</p>
                    </div>
            ...
            <div class="bg-gradient-to-br from-white to-amber-50/30 rounded-3xl shadow-xl shadow-amber-200/50 p-8 border border-amber-100/50 hover:shadow-2xl hover:shadow-amber-200/60 transition-all duration-500 group">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-slate-500 text-sm font-semibold uppercase tracking-wider">Vacants</p>
                        <p class="text-4xl font-bold bg-gradient-to-r from-amber-600 to-amber-500 bg-clip-text text-transparent mt-3">{{ stats.vacant }}</p>
                    </div>
                    <div class="w-16 h-16 bg-gradient-to-br from-amber-500 via-amber-600 to-orange-600 rounded-2xl flex items-center justify-center shadow-lg shadow-amber-500/30 group-hover:scale-110 transition-transform duration-300">
                        <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16l2.879-2.879m0 0a3 3 0 104.243-4.243m-4.243 4.243L3 3m19.414 19.414a9 9 0 10-12.728-12.728" />
                        </svg>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-white to-violet-50/30 rounded-3xl shadow-xl shadow-violet-200/50 p-8 border border-violet-100/50 hover:shadow-2xl hover:shadow-violet-200/60 transition-all duration-500 group">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-slate-500 text-sm font-semibold uppercase tracking-wider">Loyer Total</p>
                ...
                <div class="flex-1 flex gap-4 w-full lg:w-auto">
                    <div class="relative flex-1">
                        <input
                            ...
                            type="text"
                            placeholder="Rechercher par numéro, étage, type..."
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead class="bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200">
                        <tr>
                            <th class="px-6 py-4 text-left text-sm font-semibold text-slate-700">N° Unité</th>
                            <th class="px-6 py-4 text-left text-sm font-semibold text-slate-700">Bâtiment</th>
                            <th class="px-6 py-4 text-left text-sm font-semibold text-slate-700">Étage</th>
                    ...
                    <tbody class="divide-y divide-slate-200">
                        <tr v-for="apt in filteredApartments" :key="apt.id" class="hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-4 text-sm font-medium text-slate-900">{{ apt.number }}</td>
                            <td class="px-6 py-4 text-sm text-slate-600">{{ apt.building }}</td>
                            <td class="px-6 py-4 text-sm text-slate-600">{{ apt.floor }}</td>
                            <td class="px-6 py-4 text-sm text-slate-600">{{ apt.type }}</td>
                            <td class="px-6 py-4 text-sm font-semibold text-slate-900">{{ formatCurrency(apt.rent) }}</td>
                            <td class="px-6 py-4 text-sm text-slate-600">{{ apt.tenant || '-' }}</td>
                            <td class="px-6 py-4 text-sm">
                                <span class="px-3 py-1 rounded-full text-xs font-bold"
                                    :class="apt.status === 'occupied' 
                                        ? 'bg-emerald-100 text-emerald-700' 
                                        : 'bg-amber-100 text-amber-700'">
                                    {{ apt.status === 'occupied' ? 'Occupé' : 'Vacant' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-sm">
                                <div class="flex gap-2 justify-center">
                                    <button @click="editApartment(apt)" class="text-blue-600 hover:text-blue-700 hover:bg-blue-50 p-2 rounded-lg transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg>
                                    </button>
                                    <button @click="deleteApartment(apt)" class="text-red-600 hover:text-red-700 hover:bg-red-50 p-2 rounded-lg transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div v-if="showModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div class="bg-white rounded-3xl shadow-2xl max-w-2xl w-full p-8 max-h-[90vh] overflow-y-auto">
                <div class="flex items-center justify-between mb-6 sticky top-0 bg-white">
                    <h2 class="text-2xl font-bold text-slate-900">{{ isEditing ? 'Modifier' : 'Créer' }} Logement</h2>
                ...
                <form @submit.prevent="saveApartment" class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">N° Unité</label>
                            <input v-model="formData.number" type="text" class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">Bâtiment</label>
                            <input v-model="formData.building" type="text" class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        </div>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">Étage</label>
                            <input v-model="formData.floor" type="number" class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">Type</label>
                            <select v-model="formData.type" class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500">
                                ...
                                <option value="T5">T5+ (5+ pièces)</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">Surface (m²)</label>
                            <input v-model="formData.surface" type="number" class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input v-model="formData.tenant" type="text" class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        </div>
                        <div>
        ...
        <div v-if="showDeleteConfirm" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div class="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8">
                ...
                <h3 class="text-xl font-bold text-center text-slate-900 mb-2">Supprimer le logement ?</h3>
...
const apartments = ref([
    { id: 1, number: 'A101', building: 'Tour Horizon', floor: 1, type: 'T2', rent: 1200, surface: 55, tenant: 'Jean Dupont', status: 'occupied', description: 'Apartment moderne' },
    { id: 2, number: 'A102', building: 'Tour Horizon', floor: 1, type: 'T3', rent: 1500, surface: 75, tenant: '', status: 'vacant', description: 'Grand apartment' },
    { id: 3, number: 'B201', building: 'Bâtiment A', floor: 2, type: 'T2', rent: 1100, surface: 50, tenant: 'Marie Martin', status: 'occupied', description: 'Vue panoramique' },
]);
...
const stats = computed(() => ({
    ...
    occupied: apartments.value.filter(a => a.status === 'occupied').length,
    vacant: apartments.value.filter(a => a.status === 'vacant').length,
    totalRent: apartments.value.filter(a => a.status === 'occupied').reduce((sum, a) => sum + a.rent, 0)
}));

const filteredApartments = computed(() => {
    return apartments.value.filter(a => {
        const matchSearch = a.number.toLowerCase().includes(filters.value.search.toLowerCase()) ||
            a.building.toLowerCase().includes(filters.value.search.toLowerCase()) ||
            a.type.toLowerCase().includes(filters.value.search.toLowerCase());
...
const openCreateModal = () => {
    ...
    formData.value = { number: '', building: '', floor: '', type: 'T2', rent: '', surface: '', tenant: '', status: 'vacant', description: '' };
...
const saveApartment = () => {
    if (!formData.value.number || !formData.value.building) {
    ...
    if (isEditing.value) {
        const index = apartments.value.findIndex(a => a.id === formData.value.id);
        apartments.value[index] = { ...formData.value };
        successMessage.value = 'Logement modifié avec succès';
    } else {
        apartments.value.push({ ...formData.value, id: Date.now() });
        successMessage.value = 'Logement créé avec succès';
    }
...
const confirmDelete = () => {
    apartments.value = apartments.value.filter(a => a.id !== apartmentToDelete.value.id);
