...
const bailleurSections = {
    ...
    suivi: [
        { label: 'Incidents', href: 'landlord.incidents.index', icon: 'alert' },
        { label: 'États des lieux', href: 'landlord.inspections.index', icon: 'clipboard' },
        { label: 'Dossier documents', href: 'landlord.documents.index', icon: 'folder' },
        { label: 'Calendrier Visites', href: 'landlord.calendar.index', icon: 'calendar' },
        { label: 'Créneaux visites', href: 'landlord.visit-slots.index', icon: 'clock' },
    ],
    finances: [
        { label: 'Révision loyers', href: 'landlord.rent-revisions.index', icon: 'trending' },
        { label: 'Cautions', href: 'landlord.deposits.index', icon: 'shield' },
        { label: 'Garanties & Assurances', href: 'landlord.insurance-guarantees.index', icon: 'umbrella' },
        { label: 'Prélèvements', href: 'landlord.subscriptions.index', icon: 'credit' },
        { label: 'Fiscal', href: 'landlord.fiscal.index', icon: 'chart' },
    ],
    communication: [
        { label: 'Messagerie Client', href: 'immo.messages', icon: 'chat' },
        { label: 'Notifications', href: 'landlord.notifications.index', icon: 'bell' },
    ],
    outils: [
        { label: 'Analyses & Rapports', href: 'landlord.reports.index', icon: 'chart' },
        { label: 'Encadrement loyers', href: 'landlord.rent-control.index', icon: 'scale' },
...
const icons = {
    ...
    umbrella: 'M3 15a4 4 0 004 4h9a5 5 0 10-.1-9.999 5.002 5.002 0 10-9.78 2.096A4.001 4.001 0 003 15z',
    ...
    scale: 'M3 6l3 1m0 0L6 3m0 3l3 9m-6 0l3-9m0 0h12m0 0l-3 9m3-9l3 1m0 0l3-9m-3 9l-3 9m6-9h-6',
};
    ...
    <div class="imo-dashboard">
        ...
        >
            ...
            <nav class="flex-1 space-y-0.5 overflow-y-auto p-3">
                <template v-if="props.role === 'bailleur'">
                    <p class="imo-section-title mb-2 mt-1 px-3">Gestion</p>
                    <Link v-for="item in bailleurSections.gestion" :key="item.label" :href="route(item.href)" class="imo-nav-link" :class="{ 'imo-nav-link-active': isActive(item.href) }">
                        <svg class="h-5 w-5 shrink-0 opacity-80" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" :d="icons[item.icon]" /></svg>
                        {{ item.label }}
                    </Link>

                    <p class="imo-section-title mb-2 mt-4 px-3">Suivi technique</p>
                    <Link v-for="item in bailleurSections.suivi" :key="item.label" :href="route(item.href)" class="imo-nav-link" :class="{ 'imo-nav-link-active': isActive(item.href) }">
                        <svg class="h-5 w-5 shrink-0 opacity-80" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" :d="icons[item.icon]" /></svg>
                        {{ item.label }}
                    </Link>

                    <p class="imo-section-title mb-2 mt-4 px-3">Finances</p>
                    <Link v-for="item in bailleurSections.finances" :key="item.label" :href="route(item.href)" class="imo-nav-link" :class="{ 'imo-nav-link-active': isActive(item.href) }">
                        <svg class="h-5 w-5 shrink-0 opacity-80" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" :d="icons[item.icon]" /></svg>
                        {{ item.label }}
                    </Link>

                    <p class="imo-section-title mb-2 mt-4 px-3">Communication</p>
                    <Link v-for="item in bailleurSections.communication" :key="item.label" :href="route(item.href)" class="imo-nav-link" :class="{ 'imo-nav-link-active': isActive(item.href) }">
                        <svg class="h-5 w-5 shrink-0 opacity-80" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" :d="icons[item.icon]" /></svg>
                        {{ item.label }}
                    </Link>

                    <p class="imo-section-title mb-2 mt-4 px-3">Outils</p>
                    <Link v-for="item in bailleurSections.outils" :key="item.label" :href="route(item.href)" class="imo-nav-link" :class="{ 'imo-nav-link-active': isActive(item.href) }">
                        <svg class="h-5 w-5 shrink-0 opacity-80" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" :d="icons[item.icon]" /></svg>
                        {{ item.label }}
                    </Link>
                </template>
                ...
                <template v-else>
                    ...
                    <Link v-for="item in particulierItems" :key="item.label" :href="route(item.href)" class="imo-nav-link" :class="{ 'imo-nav-link-active': isActive(item.href) }">
                        <svg class="h-5 w-5 shrink-0 opacity-80" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" :d="icons[item.icon]" /></svg>
                        {{ item.label }}
                    </Link>
                </template>
